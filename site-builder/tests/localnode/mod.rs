...
impl WalrusSitesClusterState {
    ...
    pub async fn new() -> anyhow::Result<Self> {
        let (sui_cluster_handle, walrus_cluster, mut admin_wallet_with_client, system_context) =
            test_cluster::E2eTestSetupBuilder::new().build().await?;
        println!(
            "sui_cluster_handle: {:#?}",
            sui_cluster_handle.clone().lock().await.wallet_path().await
        );
        println!(
        ...
        let resp =
            publish_walrus_sites(&mut admin_wallet_with_client.inner, &sui_execute_client).await?;
        ...
        let sites_config = {
            ...
            let rpc_url = sui_cluster_handle.clone().lock().await.rpc_url();
        ...
        let path = sui_cluster_handle.lock().await.wallet_path().await;
...
async fn publish_walrus_sites(
    admin_wallet_with_client: &mut WalrusSDKClient<SuiContractClient>,
    sui_execute_client: &SuiClient,
    ...
    let retriable_sui_client = admin_wallet_with_client.sui_client().sui_client();
    ...
    // test-cluster
    // Sender and gas info
    // let mut wallet = load_wallet_context_from_path(
    //     Some(
    //         sui_cluster_handle
    //             .lock()
    //             .await
    ...
    // let wallet_active_address = wallet.active_address()?;
    let wallet_active_address = admin_wallet_with_client.sui_client().address();
    ...
    //     None,
    // )?
    let signed_tx = admin_wallet_with_client
        .sui_client_mut()
        .wallet_mut()
