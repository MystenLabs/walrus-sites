...
use super::{Args, Commands, ResourceArg};
...
fn test_resource_arg_basic() {
    ...
    let result = "index.html:/index.html".parse::<ResourceArg>();
    assert!(result.is_ok());
    let ResourceArg(local, site) = result.unwrap();
    assert_eq!(local, PathBuf::from("index.html"));
    assert_eq!(site, "/index.html");
}

#[test]
fn test_resource_arg_with_whitespace() {
    // Test file name with whitespace
    let result = "my file.html:/my-file.html".parse::<ResourceArg>();
    assert!(result.is_ok());
    let ResourceArg(local, site) = result.unwrap();
    assert_eq!(local, PathBuf::from("my file.html"));
    assert_eq!(site, "/my-file.html");
}

#[test]
fn test_resource_arg_with_escaped_colon() {
    // Test file name with escaped colon
    let result = r"file\:name.html:/site/path.html".parse::<ResourceArg>();
    assert!(result.is_ok());
    let ResourceArg(local, site) = result.unwrap();
    assert_eq!(local, PathBuf::from("file:name.html"));
    assert_eq!(site, "/site/path.html");
}

#[test]
fn test_resource_arg_file_ending_with_colon() {
    // Test file ending with colon (escaped)
    let result = r"file\::/site/path.html".parse::<ResourceArg>();
    assert!(result.is_ok());
    let ResourceArg(local, site) = result.unwrap();
    assert_eq!(local, PathBuf::from("file:"));
    assert_eq!(site, "/site/path.html");
}
...
fn test_resource_arg_multiple_escaped_colons() {
    ...
    let result = r"file\:name\:test.html:/site/path.html".parse::<ResourceArg>();
    assert!(result.is_ok());
    let ResourceArg(local, site) = result.unwrap();
    assert_eq!(local, PathBuf::from("file:name:test.html"));
    assert_eq!(site, "/site/path.html");
}
...
fn test_resource_arg_site_path_with_escaped_colon() {
    ...
    let result = r"file.html:/site\:path.html".parse::<ResourceArg>();
    assert!(result.is_ok());
    let ResourceArg(local, site) = result.unwrap();
    assert_eq!(local, PathBuf::from("file.html"));
    assert_eq!(site, "/site:path.html");
}
...
fn test_resource_arg_invalid_no_colon() {
    ...
    let result = "file.html".parse::<ResourceArg>();
...
fn test_resource_arg_invalid_too_many_colons() {
    ...
    let result = "file.html:/site/path.html:/extra".parse::<ResourceArg>();
    assert!(result.is_err());
}
...
fn test_resource_arg_complex_path() {
    ...
    let result = "../path/to/file.html:/assets/styles/main.css".parse::<ResourceArg>();
    assert!(result.is_ok());
    let ResourceArg(local, site) = result.unwrap();
    assert_eq!(local, PathBuf::from("../path/to/file.html"));
    assert_eq!(site, "/assets/styles/main.css");
}
