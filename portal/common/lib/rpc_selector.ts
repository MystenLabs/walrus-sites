...
class WrappedSuiClient extends SuiClient {
    ...
    constructor(url: string, transport: SuiHTTPTransport) {
        super({transport});
        this.url = url;
    }
...
export class RPCSelector implements RPCSelectorInterface {
    ...
    constructor(rpcURLs: string[]) {
        // Initialize clients.
        this.clients = rpcURLs.map((rpcUrl) =>
            new WrappedSuiClient(
                rpcUrl,
                new SuiHTTPTransport({
                    url: rpcUrl,
                    fetch: async (input, init) => {
                        try {
                            // TODO: Add  here any Vercel caching options e.g {headers: "range_header"}, {cache: '3600'});
