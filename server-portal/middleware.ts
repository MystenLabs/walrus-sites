...
export default async function middleware(request: Request) {
    ...
    // // const subdomain = request.headers.get('host').split('.')[-1];
    // return new Response(`Hello! ASHDASD You accessed the URL: ${url}`, {
    ...
    const scopeString = url.origin;
    const scope = new URL(scopeString);

    const objectIdPath = getObjectIdLink(urlString);
    if (objectIdPath) {
        return redirectToPortalURLResponse(scope, objectIdPath);
    }

    const walrusPath = getBlobIdLink(urlString);
    if (walrusPath) {
        return redirectToAggregatorUrlResponse(scope, walrusPath);
    }
 ...
 * `/[blobid.walrus]`
 */
function getBlobIdLink(url: string): string | null {
